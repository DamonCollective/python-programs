...
import sys, json, os, re, datetime, csv, unicodedata
...
def ask_service_preference():
    ...
    result = ["parcel"]
    root = tk.Tk(); root.title("ELTA_Damon2.00 — Service Preference")
...
def ask_run_mode():
    ...
    result = ['live']
    root = tk.Tk(); root.title("ELTA_Damon2.00 — Mode")
    root.attributes('-topmost', True); root.resizable(False, False)
...
def ask_what_to_run():
    """Checklist: Shipment CSV / Thank-you Notes. Returns {'mode': 'both'|'labels'|'letters'}."""
    result = {}
    root = tk.Tk(); root.title("ELTA_Damon2.00 — What do you need?")
    root.attributes('-topmost', True); root.resizable(False, False)
    ...
    def go():
        if not labels_var.get() and not letters_var.get():
            messagebox.showwarning("Pick something", "Select at least Shipment CSV or Thank-you Notes.")
            return
...
def show_order_selection(records):
    selected = []
    root = tk.Tk(); root.title("ELTA_Damon2.00 — Order Selection")
...
class EltaShippingApp:
    def __init__(self, root, filepath, include_usa=True, mode='both',
        ...
        self.root.title("ELTA_Damon2.00 — Review & Edit")
    ...
    def start_processing(self):
        ...
        self.root.destroy()
        process_live(to_process, self.mode, self.catalog, self.service_pref)
...
def process_live(elta_records, mode, catalog, service_pref='parcel'):
    if mode == 'letters':
...
if __name__ == "__main__":
    print("ELTA_Damon2.00 — Business-Account CSV Bulk-Upload Variant")
