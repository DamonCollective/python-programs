#!/usr/bin/env python3
"""
Add bilingual descriptions to the first 30 empty products.
Skips any product that already has a description (never overwrites).

Products covered:
  36   Enid — multicolour wig
  37   Hero Red
  38   Hero Yellow
  39   Hero Purple
  41   Hero Pink
  ...
  44   Hero Blonde
  45   Hero Fuchsia
  53   Diva — red asymmetric bob with side part
  54   Nastazia — auburn bob with fringe
  55   Niobe Black
  56   Niobe Platinum Blonde
  57   Niobe Pink
  58   Niobe Red
  73   Sylvia Black
  74   Sylvia Blonde
  75   Sylvia Auburn
  76   Sylvia Light Brown
  77   Sylvia Brown
  78   Sylvia Platinum Blonde
  79   Sylvia Red
  80   Sylvia Blue
  81   Sylvia Yellow
  82   Sylvia Purple
  83   Sylvia Light Blue
  84   Sylvia Pink
  85   Sylvia Fuchsia
  86   Sylvia Brown with Streaks
  87   Sylvia Purple with Streaks
"""
...
def hero(en_color, el_fem, el_neuter):
    en = en_color.lower()
    el_adj = el_fem.lower()
    return dict(
        desc_en=(
            f"<p>The <strong>Hero {en_color} Wig</strong> is a sleek, chin-length bob with a clean center part — "
            f"a classic cut that suits any face shape. Available in many colours, here in {en}. "
            ...
            f"<ul>\n"
            f"<li>Style: chin-length bob with center part — {en}</li>\n"
        ...
        desc_el=(
            f"<p>Η <strong>Περούκα Ηρώ {el_fem}</strong> είναι ένα κομψό καρέ με χωρίστρα στη μέση — "
            f"κλασική γραμμή που ταιριάζει σε κάθε σχήμα προσώπου. Σε πολλά χρώματα, εδώ σε {el_neuter}. "
            f"Ιδανική επιλογή για cosplay, μεταμφιέσεις, απόκριες και Halloween.</p>\n"
            ...
            f"<ul>\n"
            f"<li>Στυλ: καρέ με χωρίστρα στη μέση — {el_adj}</li>\n"
        ...
        short_en=f"<p>{en_color} bob wig with center part — Hero style. Handmade in Greece. Elastic base, fits all.</p>",
        short_el=f"<p>{el_fem} καρέ περούκα με χωρίστρα στη μέση — Ηρώ. Φτιάχνεται στην Ελλάδα. Ελαστική βάση.</p>",
        meta_en=f"Hero {en_color} Bob Wig Center Part | Handmade in Greece | Alegro Athens",
        meta_el=f"Ηρώ {el_fem} Καρέ Περούκα Χωρίστρα Μέση | Alegro Αθήνα",
    )


def niobe(en_color, el_fem, el_neuter):
    en = en_color.lower()
    el_adj = el_fem.lower()
    return dict(
        desc_en=(
            f"<p>The <strong>Niobe {en_color} Wig</strong> is a long, flowing wig with a clean center part — "
            f"an elegant, feminine silhouette that frames the face beautifully. Available in many colours, here in {en}. "
            f"A wonderful choice for cosplay, theatrical performances, Halloween, and any occasion requiring long, dramatic hair.</p>\n"
            f"<p>Made in our workshop in Athens, Greece. Features a comfortable elastic base that fits head "
            f"circumferences from 42 to 63 cm. Easy to wash and comb — built to last for years.</p>\n"
            f"<ul>\n"
            f"<li>Style: long flowing hair with center part — {en}</li>\n"
            f"<li>Ideal for: cosplay, Halloween, carnival, theatre, fantasy costume</li>\n"
            f"<li>Elastic base: fits 42–63 cm head circumference</li>\n"
            f"<li>Made in Greece — custom sizes available on request</li>\n"
            f"</ul>"
        ),
        desc_el=(
            f"<p>Η <strong>Περούκα Νιόβη {el_fem}</strong> είναι μια μακριά περούκα με χωρίστρα στη μέση — "
            f"κομψή, θηλυκή γραμμή που πλαισιώνει όμορφα το πρόσωπο. Σε πολλά χρώματα, εδώ σε {el_neuter}. "
            f"Υπέροχη επιλογή για cosplay, θεατρικές παραστάσεις, απόκριες και Halloween.</p>\n"
            f"<p>Φτιάχνεται στο εργαστήριό μας στην Αθήνα. Διαθέτει ελαστική βάση που εφαρμόζει σε "
            f"περίμετρο κεφαλής από 42 έως 63 εκ. Πλένεται και χτενίζεται εύκολα — φτιαγμένη για να αντέχει χρόνια.</p>\n"
            f"<ul>\n"
            f"<li>Στυλ: μακριά με χωρίστρα στη μέση — {el_adj}</li>\n"
            f"<li>Κατάλληλη για: cosplay, Halloween, απόκριες, θέατρο, φαντασιακή στολή</li>\n"
            f"<li>Ελαστική βάση: εφαρμόζει σε περίμετρο κεφαλής 42–63 εκ.</li>\n"
            f"<li>Κατασκευάζεται στην Ελλάδα — διαθέσιμη κατά παραγγελία σε ειδικά μεγέθη</li>\n"
            f"</ul>"
        ),
        short_en=f"<p>Long {en} wig with center part — Niobe style. Handmade in Greece. Elastic base, fits all.</p>",
        short_el=f"<p>Μακριά {el_adj} περούκα με χωρίστρα στη μέση — Νιόβη. Φτιάχνεται στην Ελλάδα. Ελαστική βάση.</p>",
        meta_en=f"Niobe {en_color} Long Wig Center Part | Handmade in Greece | Alegro Athens",
        meta_el=f"Νιόβη {el_fem} Μακριά Περούκα Χωρίστρα Μέση | Alegro Αθήνα",
    )
...
def sylvia(en_color, el_fem, el_neuter):
    ...
    return dict(
        ...
        short_en=f"<p>Long {en} wig with center part — Sylvia style. Handmade in Greece. Elastic base, fits all.</p>",
        short_el=f"<p>Μακριά {el_adj} περούκα με χωρίστρα στη μέση — Σύλβια. Φτιάχνεται στην Ελλάδα. Ελαστική βάση.</p>",
        meta_en=f"Sylvia {en_color} Long Wig Center Part | Handmade in Greece | Alegro Athens",
        meta_el=f"Σύλβια {el_fem} Μακριά Περούκα Χωρίστρα Μέση | Alegro Αθήνα",
    )
...
PRODUCTS = {
    ...
    36: dict(
        desc_en=(
            "<p>The <strong>Enid Multicolour Wig</strong> is a bold, eye-catching theatrical wig featuring "
            "a spectacular blend of vivid colours. A fun, theatrical choice that makes an immediate impression — "
            "perfect for carnival, fancy dress, and stage performances.</p>\n"
            "<p>Made in our workshop in Athens, Greece. Features a comfortable elastic base that fits head "
            "circumferences from 42 to 63 cm. Easy to wear — built to last for years.</p>\n"
            "<ul>\n"
            "<li>Style: multicolour theatrical wig — vivid, fun, unmissable</li>\n"
            "<li>Ideal for: carnival, cosplay, theatre, school events, Halloween</li>\n"
            "<li>Elastic base: fits 42–63 cm head circumference</li>\n"
            "<li>Made in Greece — custom sizes available on request</li>\n"
            "</ul>"
        ),
        desc_el=(
            "<p>Η <strong>Πολύχρωμη Περούκα Enid</strong> είναι μια εντυπωσιακή θεατρική περούκα με "
            "γοητευτικό συνδυασμό ζωηρών χρωμάτων. Διασκεδαστική επιλογή που τραβάει αμέσως το βλέμμα — "
            "ιδανική για αποκριές, μεταμφιέσεις και σκηνικές εμφανίσεις.</p>\n"
            "<p>Φτιάχνεται στο εργαστήριό μας στην Αθήνα. Διαθέτει ελαστική βάση που εφαρμόζει σε "
            "περίμετρο κεφαλής από 42 έως 63 εκ. Εύκολη στη χρήση — φτιαγμένη για να αντέχει χρόνια.</p>\n"
            "<ul>\n"
            "<li>Στυλ: πολύχρωμη θεατρική περούκα — ζωηρή, διασκεδαστική, εντυπωσιακή</li>\n"
            "<li>Κατάλληλη για: αποκριές, cosplay, θέατρο, σχολικές εκδηλώσεις, Halloween</li>\n"
        ...
        short_en="<p>Bold multicolour theatrical wig — Enid. Vivid colours, handmade in Greece. Elastic base, fits all.</p>",
        short_el="<p>Εντυπωσιακή πολύχρωμη θεατρική περούκα Enid. Φτιάχνεται στην Ελλάδα. Ελαστική βάση, εφαρμόζει σε όλους.</p>",
        meta_en="Enid Multicolour Theatrical Wig | Handmade in Greece | Alegro Athens",
        meta_el="Enid Πολύχρωμη Θεατρική Περούκα | Alegro Αθήνα",
    ),

    # ── Hero bob with center part (8 colours) ─────────────────────────────────
    37: hero("Red",    "Κόκκινη",        "κόκκινο"),
    38: hero("Yellow", "Κίτρινη",        "κίτρινο"),
    39: hero("Purple", "Μωβ",            "μωβ"),
    41: hero("Pink",   "Ροζ",            "ροζ"),
    42: hero("Black",  "Μαύρη",          "μαύρο"),
    43: hero("Auburn", "Καστανοκόκκινη", "καστανοκόκκινο"),
    44: hero("Blonde", "Ξανθιά",         "ξανθό"),
    45: hero("Fuchsia","Φούξια",         "φούξια"),

    # ── Diva red asymmetric bob with side part ────────────────────────────────
    53: dict(
        desc_en=(
        ...
        desc_el=(
            ...
            "χωρίστρα στο πλάι — δραματικό, μοντέρνο στυλ για όσους αγαπούν να ξεχωρίζουν. Το ασύμμετρο "
        ...
        short_en="<p>Red asymmetric bob wig with side part — Diva. Bold and dramatic. Handmade in Greece. Elastic base, fits all.</p>",
    ...
    # ── Nastazia auburn bob with fringe ───────────────────────────────────────
    54: dict(
        desc_en=(
            "<p>The <strong>Nastazia Auburn Wig</strong> is a rich, warm auburn bob with a neat fringe — "
            "a classic, feminine style that flatters any face shape. The warm chestnut-red tone gives a natural, "
            "lively look, while the short fringe adds a playful touch. A distinctive single-colour style in this "
            "beautiful auburn shade.</p>\n"
            "<p>Made in our workshop in Athens, Greece. Features a comfortable elastic base that fits head "
            "circumferences from 42 to 63 cm. Easy to wash and comb — built to last for years.</p>\n"
            "<ul>\n"
            "<li>Style: auburn bob with fringe</li>\n"
        ...
        desc_el=(
            "<p>Η <strong>Περούκα Ναστάζια Καστανοκόκκινη</strong> είναι ένα πλούσιο, ζεστό καστανοκόκκινο "
            "καρέ με φράντζα — κλασικό, θηλυκό στυλ που κολακεύει κάθε σχήμα προσώπου. Η ζεστή καστανοκόκκινη "
            "απόχρωση δίνει φυσικό, ζωντανό αποτέλεσμα, ενώ η κοντή φράντζα προσθέτει μια παιχνιδιάρικη πινελιά.</p>\n"
            "<p>Φτιάχνεται στο εργαστήριό μας στην Αθήνα. Διαθέτει ελαστική βάση που εφαρμόζει σε "
            "περίμετρο κεφαλής από 42 έως 63 εκ. Πλένεται και χτενίζεται εύκολα — φτιαγμένη για να αντέχει χρόνια.</p>\n"
            "<ul>\n"
            "<li>Στυλ: καστανοκόκκινο καρέ με φράντζα</li>\n"
            "<li>Κατάλληλη για: cosplay, Halloween, απόκριες, retro / vintage look</li>\n"
            "<li>Ελαστική βάση: εφαρμόζει σε περίμετρο κεφαλής 42–63 εκ.</li>\n"
            "<li>Κατασκευάζεται στην Ελλάδα — διαθέσιμη κατά παραγγελία σε ειδικά μεγέθη</li>\n"
            "</ul>"
        ),
        short_en="<p>Auburn bob wig with fringe — Nastazia. Warm chestnut-red shade. Handmade in Greece. Elastic base, fits all.</p>",
        short_el="<p>Καστανοκόκκινη καρέ περούκα με φράντζα — Ναστάζια. Φτιάχνεται στην Ελλάδα. Ελαστική βάση.</p>",
        meta_en="Nastazia Auburn Bob Wig with Fringe | Handmade in Greece | Alegro Athens",
        meta_el="Ναστάζια Καστανοκόκκινη Καρέ Περούκα Φράντζα | Alegro Αθήνα",
    ),

    # ── Niobe long with center part (4 colours) ───────────────────────────────
    55: niobe("Black",           "Μαύρη",          "μαύρο"),
    56: niobe("Platinum Blonde", "Πλατινέ Ξανθιά", "πλατινέ ξανθό"),
    57: niobe("Pink",            "Ροζ",             "ροζ"),
    58: niobe("Red",             "Κόκκινη",         "κόκκινο"),

    # ── Sylvia long with center part (15 colours) ─────────────────────────────
    73: dict(
        desc_en=(
