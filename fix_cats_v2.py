...
Strategy: grab ALL existing form inputs, add 3 new category fields, submit.
Do NOT override product[_token] (use the one from the page form).
Do NOT add default_category][value] (wrong field name — form uses default_category_id).
"""
...
def get_all_inputs(html):
    """Extract ALL form inputs (hidden, text, etc.) preserving their values."""
    inputs = {}
...
for pid in PRODUCTS:
    ...
    # Start from ALL page inputs (correct product[_token] already included)
    payload = get_all_inputs(r_edit.text)

    # Add Theatrical category fields only
    ...
    # Add URL-level CSRF token
    payload['_token'] = edit_tok

    # Sanity: do NOT add default_category][value] — form uses default_category_id
    # The existing default_category_id from the page is already in payload via get_all_inputs

    prod_name = payload.get('product[header][name][1]', f'Product {pid}')
    print(f'  Product {pid} ({prod_name[:45]}):')
    print(f'    fields={len(payload)}, new cat idx={next_idx}, existing={list(existing.values())}')
    ...
    print(f'    Save: {r_save.status_code} → {r_save.url[-60:]}')
