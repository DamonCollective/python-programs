#!/usr/bin/env python3
"""
Fix Daphne (139-142): remove Φουντωτή from EL product name → Σγουρή
"""
...
# Corrected names and descriptions (no φουντωτή anywhere)
PRODUCTS = {
    139: dict(
        name_en="Daphne Black Curly Wig",
    ...
    140: dict(
        name_en="Daphne Blonde Curly Wig",
    ...
    142: dict(
        name_en="Daphne Light Brown Curly Wig",
        name_el="Δάφνη Σγουρή Ανοιχτή Καστανή Περούκα",
        desc_en=(
            "<p>The <strong>Daphne Light Brown Curly Wig</strong> is a full, voluminous curly wig — "
            "a bold style with plenty of body and presence. Available in many colours, here in light brown. "
            "A fun choice for cosplay, Halloween, carnival, and any occasion that calls for a striking, high-volume look.</p>\n"
            "<p>Made in our workshop in Athens, Greece. Features a comfortable elastic base that fits head circumferences from 42 to 63 cm. Easy to wear — built to last for years.</p>\n"
            "<ul>\n<li>Style: full curly wig — light brown</li>\n"
            "<li>Ideal for: cosplay, Halloween, carnival, retro / vintage look, fancy dress</li>\n"
            "<li>Elastic base: fits 42–63 cm head circumference</li>\n"
            "<li>Made in Greece — custom sizes available on request</li>\n</ul>"
        ),
        desc_el=(
            "<p>Η <strong>Περούκα Δάφνη Ανοιχτή Καστανή</strong> είναι μια σγουρή περούκα με μεγάλο, πλούσιο όγκο — "
            "τολμηρό στυλ με δυναμική παρουσία. Σε πολλά χρώματα, εδώ σε ανοιχτό καστανό. "
            "Διασκεδαστική επιλογή για cosplay, Halloween, αποκριές και κάθε στιγμή που θέλετε εντυπωσιακή, σγουρή εμφάνιση.</p>\n"
            "<p>Φτιάχνεται στο εργαστήριό μας στην Αθήνα. Διαθέτει ελαστική βάση που εφαρμόζει σε περίμετρο κεφαλής από 42 έως 63 εκ. Εύκολη στη χρήση — φτιαγμένη για να αντέχει χρόνια.</p>\n"
            "<ul>\n<li>Στυλ: σγουρή περούκα — ανοιχτή καστανή</li>\n"
            "<li>Κατάλληλη για: cosplay, Halloween, αποκριές, retro / vintage look, μεταμφιέσεις</li>\n"
            "<li>Ελαστική βάση: εφαρμόζει σε περίμετρο κεφαλής 42–63 εκ.</li>\n"
            "<li>Κατασκευάζεται στην Ελλάδα — διαθέσιμη κατά παραγγελία σε ειδικά μεγέθη</li>\n</ul>"
        ),
        short_en="<p>Full curly light brown wig — Daphne style. Bold, high-volume look. Handmade in Greece. Elastic base, fits all.</p>",
        short_el="<p>Σγουρή ανοιχτή καστανή περούκα — Δάφνη. Τολμηρό, ογκώδες στυλ. Φτιάχνεται στην Ελλάδα. Ελαστική βάση.</p>",
        meta_en="Daphne Light Brown Curly Wig | Handmade in Greece | Alegro Athens",
        meta_el="Δάφνη Ανοιχτή Καστανή Σγουρή Περούκα | Alegro Αθήνα",
    ),
}
...
def update_product(s, cat_tok, pid, content):
    try:
        ...
        # Explicitly set corrected names
...
log(f'Fixing product titles for {len(PRODUCTS)} Daphne products.\n')

tasks = [(i % WORKERS, pid) for i, pid in enumerate(sorted(PRODUCTS.keys()))]
