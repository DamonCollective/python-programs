#!/usr/bin/env python3
"""
Fix Daphne (139-141): remove Φουντωτή from EL product name → Σγουρή
Keep EN name as scraped. Descriptions already fixed.
"""
...
# Only 139-141 have Φουντωτή in EL name; 142 EL name is "DAPHNES LIGHT BROWN"
EL_NAMES = {
    139: "Δάφνη Σγουρή Μαύρη Περούκα",
    140: "Δάφνη Σγουρή Ξανθιά Περούκα",
    141: "Δάφνη Σγουρή Κόκκινη Περούκα",
}

# Descriptions are already fixed; resend to ensure consistency
def desc(pid):
    data = {
        139: ("black",        "μαύρη",           "μαύρο"),
        140: ("blonde",       "ξανθιά",          "ξανθό"),
        141: ("red",          "κόκκινη",         "κόκκινο"),
    }
    en, el_adj, el_neut = data[pid]
    return dict(
        desc_en=(
            f"<p>The <strong>Daphne {en.title()} Curly Wig</strong> is a full, voluminous curly wig — "
        ...
        desc_el=(
            f"<p>Η <strong>Περούκα Δάφνη {el_adj.title()}</strong> είναι μια σγουρή περούκα με μεγάλο, πλούσιο όγκο — "
            f"τολμηρό στυλ με δυναμική παρουσία. Σε πολλά χρώματα, εδώ σε {el_neut}. "
        ...
        meta_en=f"Daphne {en.title()} Curly Wig | Handmade in Greece | Alegro Athens",
        meta_el=f"Δάφνη {el_adj.title()} Σγουρή Περούκα | Alegro Αθήνα",
    )
...
def update_product(s, cat_tok, pid):
    content = desc(pid)
    try:
        ...
        # Show what we scraped for name fields
        for lang in ['1', '2']:
            k = f'product[details][name][{lang}]'
            log(f'  PID {pid} scraped name[{lang}]: {payload.get(k, "<not found>")!r}')

        # Fix EL name: override with corrected version
        payload['product[details][name][2]'] = EL_NAMES[pid]
...
log(f'Fixing EL product titles for {len(EL_NAMES)} Daphne products.\n')

pids = sorted(EL_NAMES.keys())
tasks = [(i % WORKERS, pid) for i, pid in enumerate(pids)]

def worker(args):
    idx, pid = args
    s, cat = sessions[idx]
    return update_product(s, cat, pid)
