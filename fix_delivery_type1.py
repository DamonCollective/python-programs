"""
Fix: set delivery_time_note_type=1 (use global default) for all products.
type=0 = disabled (nothing shown) ← what we wrongly set
type=1 = use global default ("Παράδοση σε 1 - 2 ημέρες") ← correct
type=2 = per-product specific text
Also keeps visibility='both'.
"""
...
EXCLUDE = {
    ...
    'product[options][visibility][visibility]',  # set explicitly below
}
...
def fix_product(s, cat_token, pid):
    try:
        ...
        payload['product[shipping][delivery_time_note_type]'] = '1'  # use global default
...
log(f'Found {len(pids)} products. Setting delivery_time_note_type=1 (use global default)...\n')
...
with ThreadPoolExecutor(max_workers=WORKERS) as ex:
    ...
    for fut in as_completed(futures):
        ...
        if done % 50 == 0:
            log(f'[{done:3d}/{len(pids)}] {ok} done so far...')

elapsed = time.time() - start
log(f'\nDone in {elapsed:.0f}s — {ok} updated, {fail} failed')
