#!/usr/bin/env python3
"""
Upload 3 Gandalf wig images (model Paul) to product 382 on alegro.gr.
Product: Σετ Περούκα και Γενειάδα Γκρίζου Μάγου
"""
...
IMAGE_PATHS = [
    os.path.join(DOWNLOADS, "gandalf-the-gray-grey-wig-and-beard-set-cosplay-stage-on-model-Paul1.png"),
    os.path.join(DOWNLOADS, "gandalf-the-gray-grey-wig-and-beard-set-cosplay-stage-on-model-Paul2.png"),
    os.path.join(DOWNLOADS, "gandalf-the-gray-grey-wig-and-beard-set-cosplay-stage-on-model-Paul3.png"),
]
...
def extract_url_token(url):
    # Prefer _token (Symfony) over token (legacy)
    m = re.search(r'[?&]_token=([A-Za-z0-9._\-]+)', url)
    if m:
        return m.group(1)
...
# Extract image-form CSRF token
img_dt_m = re.search(r'data-form-name="product_image"[^>]*data-token="([^"]+)"', r2.text)
if not img_dt_m:
    img_dt_m = re.search(r'data-token="([^"]+)"[^>]*data-form-name="product_image"', r2.text)
...
print(f"\n[4] Compressing and uploading {len(IMAGE_PATHS)} images to product {PRODUCT_ID}...")
...
print(f"\n  Product URL: {ADMIN_BASE}/index.php/sell/catalog/products/{PRODUCT_ID}/edit")
print(f"{'═'*60}")
